<?php
/**
 * Chum theme functions and definitions
 *
...
function bp_dtheme_setup() {
	...
	add_theme_support( 'custom-background', apply_filters( 'cepheus_custom_background_args', array(
		'default-color' => 'ffffff',
		'default-image' => get_template_directory() . '/_inc/images/geo_pattern.png',
	) ) );
...
function bp_dtheme_enqueue_styles() {
	...
	// Register our font stylesheets
	wp_enqueue_style( 'bp-font-awesome', get_template_directory_uri() . '/_inc/fonts/font-awesome.css', array(), bp_get_version() );
	wp_enqueue_style( 'bp-good-dog', get_template_directory_uri() . '/_inc/fonts/good-dog.css', array(), bp_get_version() );
	wp_enqueue_style( 'bp-open-sans', get_template_directory_uri() . '/_inc/fonts/opensans.css', array(), bp_get_version() );
	...
	//Enqeue flexslider
	wp_enqueue_script( 'dtheme-flexslider-js', get_template_directory_uri() . '/_inc/jquery.flexslider-min.js', array( 'jquery' ), bp_get_version() );
...
function bp_dtheme_widgets_init() {
	...
	register_sidebar( array(
		'name'          => 'Front Widgets',
		'id'            => 'front-widget',
		'description'   => __( 'The front widget area', 'buddypress' ),
...
function bp_dtheme_content_nav( $nav_id ) {
	...
	if ( !empty( $wp_query->max_num_pages ) && $wp_query->max_num_pages > 1 ) : ?>
		...
		<div id="<?php echo $nav_id; ?>" class="navigation">
			<div class="alignleft"><?php next_posts_link( __( '<i class="fa fa-long-arrow-left"></i>', 'buddypress' ) ); ?></div>
			<div class="alignright"><?php previous_posts_link( __( '<i class="fa fa-long-arrow-right"></i>', 'buddypress' ) ); ?></div>
		</div><!-- #<?php echo $nav_id; ?> -->
 ...
 * Shortcodes
 */
require get_template_directory() . '/_inc/shortcodes.php';

/**
 * Widgets
 */
require get_template_directory() . '/_inc/widgets/widget-shortcode.php';


/**
 * Alter the main query loop, don't use in other query loop instead use WP_Query
 */
function bn_alter_query( $query ) {
